...
import { useData, LogEntry } from "../DataContext";
...
export function SystemLogs() {
  ...
  const filtered = useMemo(() => {
    ...
    if (moduleFilter !== "all-modules") {
      list = list.filter((l: LogEntry) => l.module.toLowerCase().includes(moduleFilter));
    }
    if (statusFilter !== "all-status") {
      list = list.filter((l: LogEntry) => l.status === statusFilter);
    }
    return list;
  }, [logs, search, moduleFilter, statusFilter]);

  const totalLogs = logs.length;
  const successCount = logs.filter((l: LogEntry) => l.status === "success").length;
  const warningCount = logs.filter((l: LogEntry) => l.status === "warning").length;
  const errorCount = logs.filter((l: LogEntry) => l.status === "error").length;
