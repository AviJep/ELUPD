...
interface HousingTableProps {
  data: HousingProject[];
  onView?: (item: HousingProject) => void;
  onEdit?: (item: HousingProject) => void;
  onArchive?: (item: HousingProject) => void;
  onAdd?: () => void;
  onImport?: (data: any[]) => void;
}

type SortKey = keyof HousingProject;

export default function HousingTable({ data, onView, onEdit, onArchive, onAdd, onImport }: HousingTableProps) {
  ...
  const handleExport = () => {
    exportToCSV(sortedData, `Housing_Projects_${new Date().toISOString().split('T')[0]}.csv`);
  };
    ...
    <div className="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-200">
        ...
        <table className="w-full text-sm text-left text-gray-700">
          <thead className="bg-gray-50/50 text-[#003087] font-black uppercase text-[10px] tracking-widest border-b border-gray-100">
            <tr>
              ...
              <SortableHeader label="Project Name" sortKey="projectName" currentSort={sortKey} order={sortOrder} onClick={() => handleSort("projectName")} />
              <SortableHeader label="Developer" sortKey="developer" currentSort={sortKey} order={sortOrder} onClick={() => handleSort("developer")} />
              <SortableHeader label="Type" sortKey="projectType" currentSort={sortKey} order={sortOrder} onClick={() => handleSort("projectType")} />
            ...
            {currentData.length > 0 ? (
              currentData.map((row) => (
                <tr key={row.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="p-4 font-bold text-gray-900">{row.cityMunicipality}</td>
                  <td className="p-4 font-black text-[#003087] group-hover:underline transition-all cursor-pointer">{row.projectName}</td>
                  <td className="p-4 font-medium text-gray-600">{row.developer}</td>
                  <td className="p-4 text-[11px] font-bold text-gray-400 uppercase">{row.projectType}</td>
                  <td className="p-4">
                    <span className="px-2 py-1 bg-gray-100 rounded-full text-[10px] font-black uppercase text-gray-500 border border-gray-200">
                      {row.status}
                    </span>
                  </td>
                  <td className="p-4">
              ...
              <tr>
                <td colSpan={8} className="p-20 text-center text-gray-400 font-medium">No records found matching your criteria.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="p-4 flex flex-col sm:flex-row justify-between items-center bg-gray-50/50 border-t border-gray-100 gap-4">
        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
          Showing <span className="text-[#003087] font-black">{sortedData.length > 0 ? startIndex + 1 : 0}</span> - <span className="text-[#003087] font-black">{Math.min(startIndex + itemsPerPage, sortedData.length)}</span> of <span className="text-[#003087] font-black">{sortedData.length}</span> projects
        </span>
