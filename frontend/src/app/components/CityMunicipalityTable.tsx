...
interface CityMunicipalityTableProps {
  data: CityMunicipality[];
  onView?: (item: CityMunicipality) => void;
  onEdit?: (item: CityMunicipality) => void;
  onArchive?: (item: CityMunicipality) => void;
  onAdd?: () => void;
  onImport?: (data: any[]) => void;
}

type SortKey = keyof CityMunicipality;

export default function CityMunicipalityTable({ data, onView, onEdit, onArchive, onAdd, onImport }: CityMunicipalityTableProps) {
  ...
  const handleExport = () => {
    exportToCSV(sortedData, `LGU_Directory_${new Date().toISOString().split('T')[0]}.csv`);
  };
    ...
    <div className="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-200">
        ...
        <table className="w-full text-sm text-left text-gray-700">
          <thead className="bg-gray-50/50 text-[#003087] font-black uppercase text-[10px] tracking-widest border-b border-gray-100">
            <tr>
              ...
              <SortableHeader label="LGU Type" sortKey="lguType" currentSort={sortKey} order={sortOrder} onClick={() => handleSort("lguType")} />
              <SortableHeader label="Income Class" sortKey="incomeClass" currentSort={sortKey} order={sortOrder} onClick={() => handleSort("incomeClass")} />
            ...
            {currentData.length > 0 ? (
              currentData.map((row) => (
                <tr key={row.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="p-4 font-bold text-blue-600/70">{row.lguType}</td>
                  <td className="p-4 font-bold text-amber-600/70">{row.incomeClass}</td>
                  <td className="p-4">
              ...
              <tr>
                <td colSpan={6} className="p-20 text-center text-gray-400 font-medium">No records found matching your criteria.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="p-4 flex flex-col sm:flex-row justify-between items-center bg-gray-50/50 border-t border-gray-100 gap-4">
        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
          Showing <span className="text-[#003087] font-black">{sortedData.length > 0 ? startIndex + 1 : 0}</span> - <span className="text-[#003087] font-black">{Math.min(startIndex + itemsPerPage, sortedData.length)}</span> of <span className="text-[#003087] font-black">{sortedData.length}</span> LGUs
        </span>
